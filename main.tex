...
regression and time-varying endpoint models. The construction updates a prior
with a product-residual check loss, rather than with a response likelihood. A
...
regularization and uncertainty summaries for endpoint functions. It is not a
sampling model for responses. Consequently, posterior endpoint draws should not
be read as posterior predictive response draws, and repeated-sampling tolerance
...
The scope is deliberately bounded. Nonzero tilt is treated as fixed before
...
\eqref{eq:generalized-posterior}. It does not turn the product residual into a
response likelihood for \(Y_i\).
